...
class MockCharacteristicStore {
  ...
  constructor() {
    ...
    if (savedState) {
      ...
      Object.entries(savedState.characteristicValues).forEach(
        ([uuid, value]) => {
...
export class MockDevice implements Partial<Device> {
  ...
  private disconnectCallback:
    | ((error: BleError | null, device: Device) => void)
    | null = null;
  ...
  async requestMTU(size: number): Promise<Device> {
    console.log(`Mock: Requesting MTU size ${size}`);
  ...
  private async handleCharacteristicWrite(
    ...
    switch (uuid) {
      ...
      case OTA_UUID:
        await this.handleOTA();
        break;
    }
  }

  private async handleHeadlightCommand(
    command: DefaultCommandValue,
  ): Promise<void> {
    this.animationQueue = this.animationQueue.then(async () => {
      const customCommandActive =
        mockStore.getValue(CUSTOM_COMMAND_UUID) === "1";

      if (!customCommandActive) {
        mockStore.setValue(BUSY_CHAR_UUID, "1");
      }
      try {
        const headlightMotionDuration = parseInt(
          mockStore.getValue(HEADLIGHT_MOTION_IN_UUID),
        );

        switch (command) {
          case DefaultCommandValue.LEFT_UP:
            await this.animateStatus(
              LEFT_STATUS_UUID,
              ButtonStatus.UP,
        ...
        for (const part of parts) {
          ...
          if (part.startsWith("d")) {
            ...
            if (!isNaN(command)) {
              ...
              await this.executeHeadlightCommand(
                command,
                headlightMotionDuration,
              );
            }
  ...
  private async executeHeadlightCommand(
    command: DefaultCommandValue,
    motionTime: number,
  ): Promise<void> {
  ...
  private async animateDirectly(
    ...
    for (let i = 1; i <= steps; i++) {
      ...
      if (i === steps) {
        mockStore.setValue(
          statusUUID,
          this.encodePercentageToValue(toPercentage).toString(),
        );
      } else {
        mockStore.setValue(
          statusUUID,
          this.encodePercentageToValue(currentPosition).toString(),
        );
      }
    }
  }

  private async animateWink(
    statusUUID: string,
    motionTime: number,
  ): Promise<void> {
  ...
  private async animateBlink(motionTime: number): Promise<void> {
    ...
    const opposite =
      leftStatus === ButtonStatus.UP ? ButtonStatus.DOWN : ButtonStatus.UP;
  ...
  private async animateWave(
    direction: "left" | "right",
    motionTime: number,
  ): Promise<void> {
    const firstUUID =
      direction === "left" ? LEFT_STATUS_UUID : RIGHT_STATUS_UUID;
    const secondUUID =
      direction === "left" ? RIGHT_STATUS_UUID : LEFT_STATUS_UUID;
    const motionDelayPercent = mockStore.getValue(
      HEADLIGHT_MOVEMENT_DELAY_UUID,
    );
    const motionDelay = parseFloat(motionDelayPercent);
    ...
    const oppositeFirst =
      firstStatus === ButtonStatus.UP ? ButtonStatus.DOWN : ButtonStatus.UP;
    const oppositeSecond =
      secondStatus === ButtonStatus.UP ? ButtonStatus.DOWN : ButtonStatus.UP;
    ...
    try {
      ...
      if (value.length > 1) {
        ...
        if (mockStore.getCustomButtonPressUpdateState() === 0) {
          ...
          console.log(
            `Mock: Set button array[${indexToUpdate}] = ${parsedValue}`,
          );
  ...
  private async handleSleepyEye(): Promise<void> {
    ...
    const headlightMotionDuration = mockStore.getValue(
      HEADLIGHT_MOTION_IN_UUID,
    );
  ...
  private async handleOTA(): Promise<void> {
