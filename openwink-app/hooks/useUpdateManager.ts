...
type UpdateData = {
  version: string; description: string; size: number;
}
...
type UpdateManagerType = {
  onSuccess: ({
    ...
    successType: UPDATE_STATUS; successMessage: string; successTitle: string;
  }) => void;
  ...
  onError: ({
    ...
    errorMessage,
    errorTitle,
  }: {errorType: ERROR_TYPE; errorMessage: string; errorTitle: string;}) =>
      void;
}
      ...
      async () => {
        ...
        if (updateSuccessStatus) {
          ...
          if (error !== ERROR_TYPE.ERR_UPDATE_HALTED) {
            // error does not update while in function. Would need ref def to
            // achieve this
            ...
            onError({
              ...
              errorMessage:
                  'Firmware update failed during installation. Reconnect to module to try again.',
